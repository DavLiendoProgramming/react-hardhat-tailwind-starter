...
const YERC20 = artifacts.require('YERC20');

const CurveDeposit = artifacts.require('CurveFi_DepositY');
const CurveSwap = artifacts.require('CurveFi_SwapY');
const CurveLPToken = artifacts.require('CurveFi_LPTokenY');
const CurveCRVMinter = artifacts.require('CurveFi_Minter');
const CurveGauge = artifacts.require('CurveFi_Gauge');
...
contract(
  'Integrate Curve.Fi into your defi',
  async ([owner, defiowner, user1, user2]) => {
    let dai;
    let usdc;
    let tusd;
    let usdt;
    ...
    before(async () => {
      ...
      await dai.methods['initialize(string,string,uint8,uint256)'](
        'DAI',
        'DAI',
        18,
        supplies.dai,
      ...
      await usdc.methods['initialize(string,string,uint8,uint256)'](
        'USDC',
        'USDC',
        6,
        supplies.usdc,
      ...
      await tusd.methods['initialize(string,string,uint8,uint256)'](
        'TUSD',
        'TUSD',
        6,
        supplies.dai,
      ...
      await usdt.methods['initialize(string,string,uint8,uint256)'](
        'USDT',
        'USDT',
        18,
        supplies.dai,
      ...
      await crvToken.methods['initialize(string,string,uint8,uint256)'](
        'CRV',
        'CRV',
        18,
        0,
      ...
      await dai.transfer(user1, new BN('1000000000000000000000'), {
        from: owner,
      });
      await usdc.transfer(user1, new BN('1000000000'), { from: owner });
      await tusd.transfer(user1, new BN('1000000000'), { from: owner });
      await usdt.transfer(user1, new BN('1000000000000000000000'), {
        from: owner,
      });

      await dai.transfer(user2, new BN('1000000000000000000000'), {
        from: owner,
      });
      await usdc.transfer(user2, new BN('1000000000'), { from: owner });
      await tusd.transfer(user2, new BN('1000000000'), { from: owner });
      await usdt.transfer(user2, new BN('1000000000000000000000'), {
        from: owner,
      });
    });
    ...
    describe('Deposit your money into Curve.Fi', () => {
      it('Deposit', async () => {
        ...
        await usdc.approve(moneyToCurve.address, deposits.usdc, {
          from: user1,
        });
        await tusd.approve(moneyToCurve.address, deposits.tusd, {
          from: user1,
        });
        await usdt.approve(moneyToCurve.address, deposits.usdt, {
          from: user1,
        });
        ...
        expect(
          daiBefore.sub(daiAfter).toString(),
          'Not deposited DAI'
        ).to.equal(deposits.dai.toString());
        expect(
          usdcBefore.sub(usdcAfter).toString(),
          'Not depositde USDC'
        ).to.equal(deposits.usdc.toString());
        expect(
          tusdBefore.sub(tusdAfter).toString(),
          'Not deposited TUSD'
        ).to.equal(deposits.tusd.toString());
        expect(
          usdtBefore.sub(usdtAfter).toString(),
          'Not deposited USDT'
        ).to.equal(deposits.usdt.toString());
      });

      it('Funds are wrapped with Y-tokens', async () => {
        expect(
          (await dai.balanceOf(ydai.address)).toString(),
          'DAI not wrapped'
        ).to.equal(deposits.dai.toString());
        expect(
          (await usdc.balanceOf(yusdc.address)).toString(),
          'USDC not wrapped'
        ).to.equal(deposits.usdc.toString());
        expect(
          (await tusd.balanceOf(ytusd.address)).toString(),
          'TUSD not wrapped'
        ).to.equal(deposits.tusd.toString());
        expect(
          (await usdt.balanceOf(yusdt.address)).toString(),
          'USDT not wrapped'
        ).to.equal(deposits.usdt.toString());
      });
      ...
      it('Y-tokens are deposited to Curve.Fi Swap', async () => {
        expect(
          (await ydai.balanceOf(curveSwap.address)).toString(),
          'YDAI not deposited'
        ).to.equal(deposits.dai.toString());
        expect(
          (await yusdc.balanceOf(curveSwap.address)).toString(),
          'YUSDC not deposited'
        ).to.equal(deposits.usdc.toString());
        expect(
          (await ytusd.balanceOf(curveSwap.address)).toString(),
          'YTUSD not deposited'
        ).to.equal(deposits.tusd.toString());
        expect(
          (await yusdt.balanceOf(curveSwap.address)).toString(),
          'YUSDT not deposited'
        ).to.equal(deposits.usdt.toString());
      });
      ...
      it('Curve.Fi LP-tokens are staked in Gauge', async () => {
        let lptokens = deposits.dai
          .add(deposits.usdc.mul(new BN('1000000000000')))
          .add(deposits.tusd.mul(new BN('1000000000000')))
          .add(deposits.usdt);
        expect(
          (await moneyToCurve.curveLPTokenStaked()).toString(),
          'Stake is absent'
        ).to.equal(lptokens.toString());
        expect(
          (await curveGauge.balanceOf(moneyToCurve.address)).toString(),
          'Stake is absent in Gauge'
        ).to.equal(lptokens.toString());
      });

      it('CRV tokens are minted and transfered to the user', async () => {
        expect(
          (await crvToken.balanceOf(user1)).toNumber(),
          'No CRV tokens'
        ).to.be.gt(0);
      });
    });
    ...
    describe('Withdraw your money from Curve.Fi', () => {
      it('Withdraw', async () => {
        ...
        expect(
          daiAfter.sub(daiBefore).toString(),
          'Not withdrawn DAI'
        ).to.equal(deposits.dai.toString());
        expect(
          usdcAfter.sub(usdcBefore).toString(),
          'Not withdrawn USDC'
        ).to.equal(deposits.usdc.toString());
        expect(
          tusdAfter.sub(tusdBefore).toString(),
          'Not withdrawn TUSD'
        ).to.equal(deposits.tusd.toString());
        expect(
          usdtAfter.sub(usdtBefore).toString(),
          'Not withdrawn USDT'
        ).to.equal(deposits.usdt.toString());
      });
    });
